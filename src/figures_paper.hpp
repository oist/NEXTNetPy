...
std::tuple<std::vector<double>, std::vector<double>,std::vector<double>> simulate_on_clustered(int SIZE,double mean,double variance,double alpha, double beta,Transmission_time psi, Transmission_time* rho= nullptr,bool SIR=false,double TMAX = 1000, bool EDGES_CONCURRENT= true,int INITIAL_INFECTED=1, int seed = 1,int NB_SIMULATIONS=1, bool TRIM=false,bool VERBOSE=false){
    rng_t engine;
    engine.seed(seed);
    if (VERBOSE)
