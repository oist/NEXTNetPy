...
// Wrapper to measure the time taken to run a simulation on a given network
template <typename T>
std::vector<double> measure_run_time_regir(py::object graph,T psi, T* rho,bool SIR,double TMAX, int INITIAL_INFECTED, int seed, int NB_SIMULATIONS,bool VERBOSE,int approximation){
    ...
    for (int sim = 0; sim < NB_SIMULATIONS; sim++){
        ...
        p.approximation_threshold = approximation;
